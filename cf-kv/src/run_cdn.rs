    ...
    ($serializer: ty, $datapath: ty, $opt: ident) => {
        ...
        let server_addr_clone =
            ...
            threads.push(std::thread::spawn(move || {
                ...
                let mut kv_client: KVClient<CdnClient, $serializer, $datapath> = KVClient::new(retwis_client, server_addr_clone, max_num_requests,opt_clone.retries, server_load_generator_opt)?;
...
#[structopt(
    name = "CDN Distribution KV Store App.",
    about = "CDN KV store server and client."
)]
pub struct CdnOpt {
    #[structopt(
        short = "debug",
