...
if [ $2 = "cfkv" ]; then
	...
	if [ $3 = "server" ]; then
		...
		cmd="sudo env RUST_BACKTRACE=1 LD_LIBRARY_PATH=/mydata/ygina/cornflakes/dpdk-datapath/3rdparty/dpdk/build/lib/x86_64-linux-gnu /mydata/ygina/cornflakes/target/release/ds_echo_dpdk --mode client --config_file $CORNFLAKES_CONFIG --server_ip 192.168.1.1 --our_ip 192.168.1.2 --debug_level info --time 15 --rate 100000 --num_threads 16 --message $message --size 4096 --push_buf_type $push_buf_type --inline_mode $inline_mode --serialization $serialization"
	else
		echo $usage
		exit 1
	fi
