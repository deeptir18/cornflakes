    ...
    D: Datapath,
{
    ...
    message_info: HashMap<MsgID, (MsgType, String)>,
    /// If in debug mode, actually load and check that the response is correct
    debug_data: HashMap<String, Vec<String>>,
    /// Check actual values
    debug_response: bool,
        ...
        if data_trace_file != "" {
            ...
            for line_res in reader.lines() {
                ...
                match req.get_type() {
                    ...
                    MsgType::Put(_) => {
                        let (key, vals) = req.get_kv()?;
                        debug_data.insert(key, vals);
                    }
                }
            }
        }
    ...
    D: Datapath,
{
    ...
    fn process_received_msg(
        ...
        if cfg!(debug_assertions) {
            if let Some((msg_type, key)) = self.message_info.remove(&sga.get_id()) {
...
where
    ...
    S: KVSerializer<D>,
{
    // TODO: for now -- application value is vector of values
    // TODO: can make this complex data structures like Redis
    map: HashMap<String, Vec<CfBuf<D>>>,
    serializer: S,
}
    ...
    pub fn load(
        ...
        for line_res in reader.lines() {
            ...
            match req.get_type() {
                ...
                MsgType::Put(_) => {
                    let (key, vals) = req.get_kv()?;
                    let mut vec: Vec<CfBuf<D>> = Vec::default();
