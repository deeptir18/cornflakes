    ...
    D: Datapath,
{
    ...
    fn total_header_size(&self, with_ref: bool) -> usize {
        <Self as CornflakesObject<D>>::CONSTANT_HEADER_SIZE * (with_ref as usize)
...
where
    D: Datapath,
{
    pub fn new(ptr: &[u8], datapath: &mut D) -> Result<Self> {
        if ptr.len() < datapath.get_copying_threshold() {
            return Ok(CFBytes::Copied(ptr.to_vec()));
        } else {
            match datapath.recover_metadata(ptr)? {
                Some(m) => Ok(CFBytes::RefCounted(m)),
                None => Ok(CFBytes::Copied(ptr.to_vec())),
            }
        }
    }
...
impl<D> CornflakesObject<D> for CFBytes<D>
where
    D: Datapath,
{
    ...
    D: Datapath,
{
    pub fn new(ptr: &[u8], datapath: &mut D) -> Result<Self> {
        if ptr.len() < datapath.get_copying_threshold() {
            return Ok(CFString::Copied(ptr.to_vec()));
        } else {
            match datapath.recover_metadata(ptr)? {
                Some(m) => Ok(CFString::RefCounted(m)),
                None => Ok(CFString::Copied(ptr.to_vec())),
            }
        }
    }
...
impl<D> CornflakesObject<D> for CFString<D>
where
    D: Datapath,
{
    const NUMBER_OF_FIELDS: usize = 1;
        ...
        F: FnMut(&D::DatapathMetadata, &mut D::CallbackEntryState) -> Result<()>,
    {
        match self {
            ...
            CFString::Copied(vec) => {
                ...
                obj_ref.write_offset((*cur_copy_offset + serialization_info.header_size) as u32);
                tracing::debug!(
                    offset_to_write = *cur_copy_offset,
                    size = vec.len(),
                    "Reached inner serialize for cf bytes"
                );
                match copy_data_buffer {
...
where
    T: CornflakesObject<D> + Clone + std::fmt::Debug,
    D: Datapath,
{
    num_space: usize,
    num_set: usize,
...
impl<T, D> CornflakesObject<D> for VariableList<T, D>
where
    T: CornflakesObject<D> + Clone + std::fmt::Debug,
    D: Datapath,
{
    ...
    fn inner_deserialize(
        ...
        for (i, elt) in self.elts.iter_mut().take(size).enumerate() {
            if elt.dynamic_header_size() == 0 {
                ...
                elt.inner_deserialize(buf, dynamic_off, buffer_offset)?;
            }
        }
        Ok(())
    }
