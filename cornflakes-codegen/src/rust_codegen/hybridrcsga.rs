...
fn add_dependencies(repr: &ProtoReprInfo, compiler: &mut SerializationCompiler) -> Result<()> {
    ...
    compiler.add_dependency("cornflakes_libos::dynamic_rcsga_hybrid_hdr::{HybridArenaRcSgaHdr}")?;
...
fn add_header_repr(
    ...
    let num_sge_function_context = FunctionContext::new(
        "num_zero_copy_scatter_gather_entries",
        false,
        vec![FunctionArg::SelfArg],
        "usize",
    );
        ...
        match &field_info.0.typ {
            FieldType::String | FieldType::Bytes | FieldType::MessageOrEnum(_) => {
                ...
                num_sge_entries = format!(
                    "{} self.{}.num_zero_copy_scatter_gather_entries()",
                    start,
                    &field_info.get_name(),
                );
            }
    ...
    let func_context = FunctionContext::new_with_lifetime(
        ...
        vec![
            ...
            FunctionArg::new_arg(
                "copy_context",
                ArgInfo::ref_mut_arg(
                    &format!("CopyContext<'a,{}>", fd.get_datapath_trait_key()),
                    None,
                ),
            ),
            FunctionArg::new_arg(
                "zero_copy_scatter_gather_entries",
                ArgInfo::ref_mut_arg(
