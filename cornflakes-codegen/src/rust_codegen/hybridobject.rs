...
fn add_dependencies(repr: &ProtoReprInfo, compiler: &mut SerializationCompiler) -> Result<()> {
    compiler.add_dependency("cornflakes_libos::{datapath::Datapath, SerializationInfo}")?;
    compiler.add_dependency("bitmaps::Bitmap")?;
    compiler.add_dependency("color_eyre::eyre::Result")?;
    compiler.add_dependency("cornflakes_libos::dynamic_object_hdr::*")?;
    compiler.add_dependency("cornflakes_libos::dynamic_object_hdr::{CornflakesObject}")?;
...
fn add_struct_definition(
    ...
    compiler.add_newline()?;
    let type_annotations = msg_info.get_type_params_hybrid_object(&fd, false)?;
    ...
    for field in msg_info.get_fields().iter() {
        ...
        compiler.add_struct_field(
            &field_info.get_name(),
            &fd.get_rust_type_hybrid_object(field_info, false)?,
        )?;
    }
...
fn add_get(
    ...
    let field_type = fd.get_rust_type_hybrid_object(field.clone(), false)?;
...
fn add_get_mut(
    ...
    let rust_type = fd.get_rust_type_hybrid_object(field.clone(), false)?;
    let func_context = FunctionContext::new(
...
fn add_header_repr(
    ...
    let struct_name = StructName::new(&msg_info.get_name(), type_annotations.clone());
    let trait_name = TraitName::new("CornflakesObject", type_annotations.clone());
    let impl_context = ImplContext::new(struct_name, Some(trait_name), where_clause);
    compiler.add_context(Context::Impl(impl_context))?;
    ...
    // add new function
    let new_function_context = FunctionContext::new("new", false, vec![], "Self where Self: Sized");

    compiler.add_context(Context::Function(new_function_context))?;
    let struct_def_context = StructDefContext::new(&msg_info.get_name());
    compiler.add_context(Context::StructDef(struct_def_context))?;
    compiler.add_struct_def_field(
        "bitmap",
    ...
    for field in msg_info.get_fields().iter() {
        ...
        if field_info.is_list() {
            if field_info.is_int_list() {
                compiler.add_struct_def_field(&field_info.get_name(), "List::new()")?;
            } else {
                compiler.add_struct_def_field(&field_info.get_name(), "VariableList::new()")?;
            }
        } else {
            compiler.add_struct_def_field(
                &field_info.get_name(),
                &fd.get_default_type_object(field_info, false)?,
            )?;
        }
    }
    compiler.add_struct_def_field("_x", "std::marker::PhantomData::default()")?;
