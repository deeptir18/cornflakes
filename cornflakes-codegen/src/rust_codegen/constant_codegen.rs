use super::{
    ...
    Context, FunctionArg, FunctionContext, ImplContext, LoopBranch, LoopContext,
    SerializationCompiler, StructContext, StructDefContext,
};
...
fn add_get(
    ...
    // Depending on type, and if it's a list or not, fill in the value
    if field.is_list() {
    } else {
    }
