...
impl ArgType {
    pub fn new(fd: &ProtoReprInfo, field: &FieldInfo) -> Result<Self> {
...
pub fn add_cf_string(compiler: &mut SerializationCompiler) -> Result<()> {
    ////////////////////////////////////////////////////////////////////////////
    // CFString_new
    let args = vec![
    ...
    compiler.add_unsafe_def_with_let(false, None, "self_",
        "Box::from_raw(self_ as *mut CFString)")?;
    compiler.add_unsafe_set("return_ptr", "self_.bytes().as_ptr()")?;
...
pub fn add_cf_bytes(compiler: &mut SerializationCompiler) -> Result<()> {
    // todo!()
    Ok(())
}
