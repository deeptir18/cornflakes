...
long custom_ice_mbind(void *start, size_t len, int mode,
...
static void custom_ice_sigbus_error(int sig)
{
	NETPERF_PANIC("couldn't map pages");
}

static void custom_ice_touch_mapping(void *base, size_t len, size_t pgsize)
{
	__sighandler_t s;
	char *pos;
	 ...
	 */
	s = signal(SIGBUS, custom_ice_sigbus_error);
...
//#endif
	...
	custom_ice_touch_mapping(addr, len, pgsize);
	return addr;
 ...
 */
void *custom_ice_mem_map_anom(void *base, size_t len, size_t pgsize, int node)
{
	unsigned long mask = (1 << node);
	return __custom_ice_mem_map_anom(base, len, pgsize, &mask, MPOL_BIND);
}
 ...
 */
void *custom_ice_mem_map_file(void *base, size_t len, int fd, off_t offset)
{
 ...
 */
void *custom_ice_mem_map_shm(mem_key_t key, void *base, size_t len, size_t pgsize,
		  bool exclusive)
{
	void *addr;
 ...
 */
int custom_ice_mem_unmap_shm(void *addr)
{
	if (shmdt(addr) == -1)
		return -errno;
	return 0;
 ...
 */
int custom_ice_mem_lookup_page_phys_addrs(void *addr, size_t len,
