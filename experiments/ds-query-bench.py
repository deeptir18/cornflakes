...
CLIENT_SIZES_TO_LOOP = [128, 512, 1024, 2048, 4096, 8192]
...
class DsQueryIteration(runner.Iteration):
    def __init__(self, client_rates, server_size,
                 size,
                 serialization,
                 server_message_type,
                 message_type,
        ...
        * client_rates: Mapping from {int, int} specifying rates and how many
        ...
        on the machine.
        * size: Total size of the data structure to be sent from the client side (not including
        additional header space the serialization library will use).
        * server_size: Total size of data structure server will send back.
        * serialization: Serialization library to use on the client side and
        server side.
    ...
    def get_server_size_string(self):
        return "server_size_{}".format(self.server_size)

    def get_message_string(self):
        return "client_{}".format(self.message_type)

    def get_server_message_string(self):
        return "server_{}".format(self.server_message)
    ...
    def get_parent_folder(self, high_level_folder):
        path = Path(high_level_folder)
        return path / self.serialization / \
            self.get_server_message_string() / \
            self.get_server_size_string() / \
...
class DsQuery(runner.Experiment):
    ...
    def get_iterations(self, total_args):
        if total_args.exp_type == "individual":
            ...
            it = DsQueryIteration(client_rates,
                                  total_args.server_size,
                                  total_args.size,
    ...
    def add_specific_args(self, parser, namespace):
        ...
        if namespace.exp_type == "individual":
            ...
            parser.add_argument("-s", "--size",
                                ...
                                required=True)
            parser.add_argument("-ss", "--server_size",
                                dest="server_size",
                                help="Total message size.",
                                required=True)
            parser.add_argument("-m", "--message_type",
                                ...
                                default="single")
            parser.add_argument("-sm", "--server_message_type",
                                dest="server_message_type",
                                choices=MESSAGE_TYPES,
                                required=True)
...
def main():
    ...
    ds_query = DsQuery(
        ...
        namespace.config)
    ds_query.execute(parser, namespace)
