pub mod ds_query_messages {
    ...
    include!(concat!(env!("OUT_DIR"), "/ds_query_fb_generated.rs"));
}
use super::{
...
use ds_query_messages::ds_query_fb;
...
fn build_tree5l<'a>(builder: &mut FlatBufferBuilder<'a>, payloads: &Vec<&'a [u8]>) {
    ...
    let tree5lfb = ds_query_fb::Tree5LFB::create(builder, &level5_args[0]);
    builder.finish(tree5lfb, None);
}

fn get_tree5l_args_from_tree5l<'a, 'b>(
    builder: &mut FlatBufferBuilder<'a>,
    input: Vec<ds_query_fb::Tree5LFB<'b>>,
) -> Vec<ds_query_fb::Tree5LFBArgs<'a>>
where
    'a: 'b,
{
    let input_tree4l_vec: Vec<ds_query_fb::Tree4LFB> = input
        .iter()
    ...
    let tree4l_offsets: Vec<WIPOffset<ds_query_fb::Tree4LFB>> = tree4l_args
        .iter()
        .map(|arg| ds_query_fb::Tree4LFB::create(builder, arg))
        .collect();
    let level5_args: Vec<ds_query_fb::Tree5LFBArgs> = (0..input.len())
        .map(|idx| ds_query_fb::Tree5LFBArgs {
...
fn get_tree5l_level_args_from_payloads_vec<'a>(
    ...
    let level4_args: Vec<WIPOffset<ds_query_fb::Tree4LFB>> = level4
        .iter()
        .map(|arg| ds_query_fb::Tree4LFB::create(builder, arg))
        .collect();

    let level5_args: Vec<ds_query_fb::Tree5LFBArgs> = (0..vec_size)
        .map(|idx| ds_query_fb::Tree5LFBArgs {
...
fn build_tree4l<'a>(builder: &mut FlatBufferBuilder<'a>, payloads: &Vec<&'a [u8]>) {
    ...
    let tree4lfb = ds_query_fb::Tree4LFB::create(builder, &level4_args[0]);
    builder.finish(tree4lfb, None);
}

fn get_tree4l_args_from_tree4l<'a, 'b>(
    builder: &mut FlatBufferBuilder<'a>,
    input: Vec<ds_query_fb::Tree4LFB<'b>>,
) -> Vec<ds_query_fb::Tree4LFBArgs<'a>>
where
    'a: 'b,
{
    let input_tree3l_vec: Vec<ds_query_fb::Tree3LFB> = input
        .iter()
    ...
    let tree3l_offsets: Vec<WIPOffset<ds_query_fb::Tree3LFB>> = tree3l_args
        .iter()
        .map(|arg| ds_query_fb::Tree3LFB::create(builder, arg))
        .collect();
    let level4_args: Vec<ds_query_fb::Tree4LFBArgs> = (0..input.len())
        .map(|idx| ds_query_fb::Tree4LFBArgs {
...
fn get_tree4l_level_args_from_payloads_vec<'a>(
    ...
    let level3_args: Vec<WIPOffset<ds_query_fb::Tree3LFB>> = level3
        .iter()
        .map(|arg| ds_query_fb::Tree3LFB::create(builder, arg))
        .collect();

    let level4_args: Vec<ds_query_fb::Tree4LFBArgs> = (0..vec_size)
        .map(|idx| ds_query_fb::Tree4LFBArgs {
...
fn build_tree3l<'a>(builder: &mut FlatBufferBuilder<'a>, payloads: &Vec<&'a [u8]>) {
    ...
    let tree3lfb = ds_query_fb::Tree3LFB::create(builder, &level3_args[0]);
    builder.finish(tree3lfb, None);
}

fn get_tree3l_args_from_tree3l<'a, 'b>(
    builder: &mut FlatBufferBuilder<'a>,
    input: Vec<ds_query_fb::Tree3LFB<'b>>,
) -> Vec<ds_query_fb::Tree3LFBArgs<'a>>
where
    'a: 'b,
{
    let input_tree2l_vec: Vec<ds_query_fb::Tree2LFB> = input
        .iter()
        .map(|tree3l| vec![tree3l.left().unwrap(), tree3l.right().unwrap()])
        .flatten()
        .collect();
    let tree2l_args = get_tree2l_args_from_tree2l(builder, input_tree2l_vec);
    let tree2l_offsets: Vec<WIPOffset<ds_query_fb::Tree2LFB>> = tree2l_args
        .iter()
        .map(|arg| ds_query_fb::Tree2LFB::create(builder, arg))
        .collect();
    let level3_args: Vec<ds_query_fb::Tree3LFBArgs> = (0..input.len())
        .map(|idx| ds_query_fb::Tree3LFBArgs {
...
fn get_tree3l_level_args_from_payloads_vec<'a>(
    ...
    let level2_args: Vec<WIPOffset<ds_query_fb::Tree2LFB>> = level2
        .iter()
        .map(|arg| ds_query_fb::Tree2LFB::create(builder, arg))
        .collect();

    let level3_args: Vec<ds_query_fb::Tree3LFBArgs> = (0..vec_size)
        .map(|idx| ds_query_fb::Tree3LFBArgs {
...
fn build_tree2l<'a>(builder: &mut FlatBufferBuilder<'a>, payloads: &Vec<&'a [u8]>) {
    ...
    let tree2lfb = ds_query_fb::Tree2LFB::create(builder, &level2_args[0]);
    builder.finish(tree2lfb, None);
}

fn get_tree2l_args_from_tree2l<'a, 'b>(
    builder: &mut FlatBufferBuilder<'a>,
    input: Vec<ds_query_fb::Tree2LFB<'b>>,
) -> Vec<ds_query_fb::Tree2LFBArgs<'a>>
where
    'a: 'b,
{
    let input_tree1l_vec: Vec<ds_query_fb::Tree1LFB> = input
        .iter()
    ...
    let tree1l_offsets: Vec<WIPOffset<ds_query_fb::Tree1LFB>> = tree1l_args
        .iter()
        .map(|arg| ds_query_fb::Tree1LFB::create(builder, arg))
        .collect();
    let level2_args: Vec<ds_query_fb::Tree2LFBArgs> = (0..input.len())
        .map(|idx| ds_query_fb::Tree2LFBArgs {
...
fn get_tree2l_level_args_from_payloads_vec<'a>(
    ...
    let level1_args: Vec<WIPOffset<ds_query_fb::Tree1LFB>> = level1
        .iter()
        .map(|arg| ds_query_fb::Tree1LFB::create(builder, arg))
        .collect();

    let level2_args: Vec<ds_query_fb::Tree2LFBArgs> = (0..vec_size)
        .map(|idx| ds_query_fb::Tree2LFBArgs {
...
fn build_tree1l<'a>(builder: &mut FlatBufferBuilder<'a>, payloads: &Vec<&'a [u8]>) {
    ...
    let tree1lfb = ds_query_fb::Tree1LFB::create(builder, &level1_args[0]);
    builder.finish(tree1lfb, None);
}

fn get_tree1l_args_from_tree1l<'a, 'b>(
    builder: &mut FlatBufferBuilder<'a>,
    input: Vec<ds_query_fb::Tree1LFB<'b>>,
) -> Vec<ds_query_fb::Tree1LFBArgs<'a>>
where
    'a: 'b,
{
    // for each tree1l, need to get the leaves
    let input_single_buffer_vec: Vec<ds_query_fb::SingleBufferFB> = input
        .iter()
    ...
    let leaf_args: Vec<WIPOffset<ds_query_fb::SingleBufferFB>> = leaves
        .iter()
        .map(|leaf| ds_query_fb::SingleBufferFB::create(builder, leaf))
        .collect();
    let level1_args: Vec<ds_query_fb::Tree1LFBArgs> = (0..input.len())
        .map(|idx| ds_query_fb::Tree1LFBArgs {
...
fn get_tree1l_level_args_from_payloads_vec<'a>(
    ...
    let level1_args: Vec<ds_query_fb::Tree1LFBArgs> = (0..vec_size)
        .map(|idx| ds_query_fb::Tree1LFBArgs {
            left: Some(leaf_args[idx * 2]),
            right: Some(leaf_args[idx * 2 + 1]),
        })
...
fn check_single_buffer<'a>(
    index: usize,
    payloads: &Vec<Vec<u8>>,
    object: &ds_query_fb::SingleBufferFB<'a>,
) {
...
fn get_leaves_from_single_buffer_fb<'a, 'b>(
    builder: &mut FlatBufferBuilder<'a>,
    input: Vec<ds_query_fb::SingleBufferFB<'b>>,
) -> Vec<ds_query_fb::SingleBufferFBArgs<'a>>
where
    'a: 'b,
{
    let leaves: Vec<ds_query_fb::SingleBufferFBArgs> = input
        .iter()
        .map(|single_buffer_fb| ds_query_fb::SingleBufferFBArgs {
...
fn get_leaves_from_payloads_vec<'a>(
    ...
    let leaves: Vec<ds_query_fb::SingleBufferFBArgs> = payloads
        .iter()
        .map(|payload| ds_query_fb::SingleBufferFBArgs {
...
fn context_size(message_type: SimpleMessageType, size: usize) -> usize {
    ...
    match message_type {
        SimpleMessageType::Single => {
            ...
            let args = ds_query_fb::SingleBufferFBArgs {
                message: Some(builder.create_vector_direct::<u8>(&payloads[0])),
            };
            let single_buffer_fb = ds_query_fb::SingleBufferFB::create(&mut builder, &args);
        ...
        SimpleMessageType::List(list_size) => {
            ...
            let mut vec: Vec<WIPOffset<ds_query_fb::SingleBufferFB>> =
                Vec::with_capacity(list_size);
            for payload in payloads.iter() {
                let args = ds_query_fb::SingleBufferFBArgs {
                    message: Some(builder.create_vector_direct::<u8>(&payload)),
                };
                vec.push(ds_query_fb::SingleBufferFB::create(&mut builder, &args));
            }
            let args = ds_query_fb::ListFBArgs {
                messages: Some(builder.create_vector(vec.as_slice())),
            };
            let list_fb = ds_query_fb::ListFB::create(&mut builder, &args);
    ...
    D: Datapath,
{
    ...
    fn process_msg<'registered, 'normal: 'registered>(
        ...
        match self.message_type {
            SimpleMessageType::Single => {
                let object_deser =
                    get_root::<ds_query_fb::SingleBufferFB>(&recved_msg.get_pkt_buffer());
                let args = ds_query_fb::SingleBufferFBArgs {
                    message: Some(ctx.create_vector_direct::<u8>(object_deser.message().unwrap())),
                };
                let single_buffer_fb = ds_query_fb::SingleBufferFB::create(ctx, &args);
                ctx.finish(single_buffer_fb, None);
                cf.add_entry(CornPtr::Normal(ctx.finished_data()));
            }
            SimpleMessageType::List(list_elts) => {
                let object_deser = get_root::<ds_query_fb::ListFB>(&recved_msg.get_pkt_buffer());
                let args_vec: Vec<ds_query_fb::SingleBufferFBArgs> = (0..list_elts)
                    .map(|idx| ds_query_fb::SingleBufferFBArgs {
                        message: Some(ctx.create_vector_direct::<u8>(
                            object_deser.messages().unwrap().get(idx).message().unwrap(),
                        )),
                    })
                    .collect();
                let vec: Vec<WIPOffset<ds_query_fb::SingleBufferFB>> = args_vec
                    .iter()
                    .map(|args| ds_query_fb::SingleBufferFB::create(ctx, args))
                    .collect();
                let list_args = ds_query_fb::ListFBArgs {
                    messages: Some(ctx.create_vector(vec.as_slice())),
                };
                let list_fb = ds_query_fb::ListFB::create(ctx, &list_args);
            ...
            SimpleMessageType::Tree(depth) => match depth {
                TreeDepth::One => {
                    let object_deser =
                        get_root::<ds_query_fb::Tree1LFB>(&recved_msg.get_pkt_buffer());
                    let args = get_tree1l_args_from_tree1l(ctx, vec![object_deser]);
                    let tree1l = ds_query_fb::Tree1LFB::create(ctx, &args[0]);
                    ctx.finish(tree1l, None);
                    cf.add_entry(CornPtr::Normal(ctx.finished_data()));
                }
                TreeDepth::Two => {
                    let object_deser =
                        get_root::<ds_query_fb::Tree2LFB>(&recved_msg.get_pkt_buffer());
                    let args = get_tree2l_args_from_tree2l(ctx, vec![object_deser]);
                    let tree2l = ds_query_fb::Tree2LFB::create(ctx, &args[0]);
                    ctx.finish(tree2l, None);
                    cf.add_entry(CornPtr::Normal(ctx.finished_data()));
                }
                TreeDepth::Three => {
                    let object_deser =
                        get_root::<ds_query_fb::Tree3LFB>(&recved_msg.get_pkt_buffer());
                    let args = get_tree3l_args_from_tree3l(ctx, vec![object_deser]);
                    let tree3l = ds_query_fb::Tree3LFB::create(ctx, &args[0]);
                    ctx.finish(tree3l, None);
                    cf.add_entry(CornPtr::Normal(ctx.finished_data()));
                }
                TreeDepth::Four => {
                    let object_deser =
                        get_root::<ds_query_fb::Tree4LFB>(&recved_msg.get_pkt_buffer());
                    let args = get_tree4l_args_from_tree4l(ctx, vec![object_deser]);
                    let tree4l = ds_query_fb::Tree4LFB::create(ctx, &args[0]);
                    ctx.finish(tree4l, None);
                    cf.add_entry(CornPtr::Normal(ctx.finished_data()));
                }
                TreeDepth::Five => {
                    let object_deser =
                        get_root::<ds_query_fb::Tree5LFB>(&recved_msg.get_pkt_buffer());
                    let args = get_tree5l_args_from_tree5l(ctx, vec![object_deser]);
                    let tree5l = ds_query_fb::Tree5LFB::create(ctx, &args[0]);
        ...
        match self.message_type {
            ...
            SimpleMessageType::List(list_size) => {
                ...
                for payload in payloads.iter() {
                    ...
                    vec.push(ds_query_fb::SingleBufferFB::create(ctx, &args));
                }
                let args = ds_query_fb::ListFBArgs {
                    messages: Some(ctx.create_vector(vec.as_slice())),
                };
                let list_fb = ds_query_fb::ListFB::create(ctx, &args);
                ctx.finish(list_fb, None);
            }
            SimpleMessageType::Tree(depth) => match depth {
                TreeDepth::One => {
