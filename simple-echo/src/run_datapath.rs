...
macro_rules! run_server (
    ($echo_server: ty, $datapath: ty, $opt: ident) => {
        ...
        let mut echo_server: $echo_server =
            <$echo_server>::new($opt.push_buf_type, $opt.request_shape);
...
macro_rules! run_client (
    ($echo_client: ty, $datapath: ty, $opt: ident) => {
        ...
        let server_addr_clone =
            ...
            threads.push(std::thread::spawn(move || {
                ...
                cornflakes_libos::state_machine::client::run_client_loadgen(i, &mut client, &mut connection, opt_clone.retries, opt_clone.total_time, opt_clone.logfile.clone(), opt_clone.rate, opt_clone.request_shape.message_size(), &schedule)
            }));
        }
